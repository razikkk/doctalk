...
export class AdminService implements IAdminService {
  ...
  constructor(AdminRepo: IAdmin,ReviewRatingRepository:IReviewRatingRepository) {
    //DInjecition(injected from outside not creating inside)
  ...
  async blockAndUnblockDoctor(
    ...
    const updateDoctor = await this.AdminRepo.blockAndUnblockDoctor(
      doctorId,
      isBlocked
    );
  ...
  async fetchDoctorReviews(doctorId: string): Promise<IReviewRating[]> {
      return await this.ReviewRatingRepository.fetchReviewPerDoctor(doctorId)
  }
}
