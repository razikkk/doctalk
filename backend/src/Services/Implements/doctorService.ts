...
export class DoctorService implements IDoctorService {
  ...
  async verificationSectionTwo(
    ...
    const doctor = await this.doctorRepository.findDoctorByEmail(email);
    const speciality = await this.adminRepository.getSpecialisationById(
      specializationId
    );
  ...
  async login(email: string, password: string): Promise<IDoctorLoginType> {
    ...
    const doctorAccessToken = generateAccessToken(
      doctor._id.toString(),
      doctor.role
    );
    console.log(jwt.decode(doctorAccessToken));
    const doctorRefreshToken = generateRefreshToken(
      doctor._id.toString(),
      doctor.role
    );
  ...
  async createDoctor(doctorData: googleUserInput): Promise<IDoctor> {
    const existingDoctor = await this.userRepository.findUserByEmail(
      doctorData.email
    );
    if (existingDoctor) {
      throw new Error("email already exists");
