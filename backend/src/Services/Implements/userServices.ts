...
import { ILoginType, IRegisterType, IUserInput, googleUserInput } from '../../type/type'
...
export class userService implements IUserService{
    ...
    async register(name:string,email:string,password:string):Promise<IRegisterType>{
                ...
                return {message:'user registered successfully',user:newUser} // ee message nmml controllerk ayakkum
            }

            async login(email:string,password:string, isAdmin:boolean = false):Promise<ILoginType>{
                        ...
                        if (user.isBlocked) {
                            ...
                            return { userData: user, accessToken: "", refreshToken: "", isBlocked: true };
                        } else {
           ...
           async validateOtp(email: string, otp: string): Promise<boolean> {
            ...
            async verifyToken(token:string):Promise<{valid:boolean,userId?:string,isBlocked?:boolean,role?:string,message?:string}>{
                try {
