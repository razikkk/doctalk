...
export class DoctorController implements IDoctorController {
  ...
  async register(
    ...
    try {
      ...
      res
        .status(201)
        .json({ success: true, message: "doctor registered", result });
    } catch (error: any) {
  ...
  async verificationSectionOne(
    ...
    try {
      ...
      return res
        .status(201)
        .json({ success: true, message: "Step 1 Completed", doctor });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  }

  async verificationSectionTwo(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void | Response> {
    try {
      ...
      const medicalCertificateUrl =
        files?.medicalCertificateUrl?.[0]?.path || "";
      ...
      return res
        .status(201)
        .json({ success: true, message: "Step 2 Completed", doctor });
    } catch (error: any) {
      console.log(error.message);
      res.status(500).json({ message: error.message });
    }
  ...
  async login(
    ...
    try {
      ...
      const { doctorData, doctorAccessToken, doctorRefreshToken } =
        await this.doctorService.login(email, password);
      ...
      return res
        ...
        .json({
          success: true,
          message: "doctor logined successfully",
          doctor: doctorData,
          doctorAccessToken,
          doctorRefreshToken,
        });
  ...
  async googleLogin(
    ...
    try {
      ...
      res
        .status(200)
        .json({ success: true, message: "doctor created", token, doctor });
    } catch (error) {
      next(error);
    }
  }
  async refreshToken(
    ...
    try {
      ...
      const newDoctorAccessToken = jwt.sign(
        { userId, role },
        ACCESS_TOKEN_SECRET,
        { expiresIn: "5d" }
      );
      ...
      return res
        .status(200)
        .json({ success: true, doctorAccessToken: newDoctorAccessToken });
    } catch (error: any) {
      console.log(error.message);
    }
  }
  async getDoctorProfile(
    ...
    try {
      ...
      res
        .status(200)
        .json({ success: true, message: "Doctor profile fetchedd", doctor });
      return;
    } catch (error: any) {
      console.log(error.message);
      res.status(500).json({ success: false, message: error.message });
  ...
  async addSlot(
    ...
    try {
      ...
      return res
        .status(201)
        .json({ success: true, message: "Slot created", slot });
    } catch (error: any) {
      console.log(error.message);
      return res.status(500).json({ success: false, message: error.message });
    }
  ...
  async editDoctorProfile(
    ...
    try {
      ...
      if (!doctorData) {
        res
          .status(401)
          .json({ success: false, message: "doctor Data is required" });
        return;
      }
      const updatedProfile = await this.doctorService.editDoctorProfile(
        doctorId,
        doctorData
      );
  ...
  async fetchDoctorAppointment(
    ...
    try {
      ...
      const slotData = await this.doctorService.fetchDoctorAppointment(
        doctorId as string
      );
      if (!doctorId) {
        return res
      ...
      if (!slotData) {
        return res
          .status(400)
          .json({ success: false, message: "slot data is required" });
      }
      return res
        .status(200)
        .json({
          success: true,
          message: "slot data fetched successfullu",
          slotData,
        });
    } catch (error: any) {
      console.log(error.message);
  ...
  async deleteSlot(
    ...
    try {
      ...
      const result = await this.doctorService.deleteSlot(slotId, true);
      if (result) {
        return res
          .status(200)
  ...
  async getAllAppointments(req: Request<ParamsDictionary, any, any, ParsedQs, Record<string, any>>, res: Response<any, Record<string, any>>, next: NextFunction): Promise<void | Response<any, Record<string, any>>> {
      try {
        const {doctorId} = req.params
        if(!doctorId){
            return res.status(400).json({success:false,message:"doctor id required"})
        }
        const result = await this.doctorService.getAllAppointments(doctorId)
        return res.status(200).json({success:true,message:"fetched appointments",result})
      } catch (error:any) {
        console.log(error.message)
        return res.status(500).json({success:false,message:error.message})
      }
  }
  async updateAppointmentStatus(req: Request, res: Response, next: NextFunction): Promise<void | Response> {
      try {
        ...
        if(!status){
            return res.status(400).json({success:false,message:"appointment status is required"})
        }
        const updatedAppointment = await this.doctorService.updateAppointmentStatus(appointmentId,status)
        return res.status(200).json({success:true,message:"appointment status updated successfully",updatedAppointment})
      } catch (error:any) {
        console.log(error.message)
        return res.status(500).json({success:false,message:error.message})
      }
  }
  async fetchReviewPerDoctor(req: Request<ParamsDictionary, any, any, ParsedQs, Record<string, any>>, res: Response<any, Record<string, any>>, next: NextFunction): Promise<void | Response<any, Record<string, any>>> {
      try {
        const {doctorId} = req.params
        if(!doctorId){
            return res.status(400).json({success:false,message:"doctor is required"})
        }
        const result = await this.doctorService.fetchReviewPerDoctor(doctorId)
        return res.status(200).json({success:true,message:"fetched the reviews",result})
      } catch (error:any) {
        console.log(error.message)
        return res.status(500).json({success:false,message:error.message})
      }
